...
export default function FriendWorldDetail() {
  const { friendId, worldId } = useParams<{ friendId: string; worldId: string }>();
  ...
  /* 从 worlds 或好友世界 mock 中查找当前好友的世界 */
  const world = useMemo(
    () => rawWorlds.find(w => w.id === worldId && w.ownerId === friendId)
      || mockFriendWorlds.find(w => w.id === worldId && w.ownerId === friendId),
    [rawWorlds, worldId, friendId],
  );

  /* 过滤当前世界的日记 */
  const entries = useMemo(
    () => rawEntries.filter(e => e.worldId === worldId),
    [rawEntries, worldId],
  );
  ...
  const friendDisplayName = friend?.friendName || '好友';

  return (
    <div className={styles.page}>
      {/* ── 顶部栏：返回 + 好友名 + 世界名（无右侧加号按钮） ── */}
      <header className={styles.topBar}>
        <button className={styles.backBtn} onClick={handleBack}>
          <span className="material-symbols-outlined">arrow_back</span>
        </button>
        <div className={styles.topBarCenter}>
          <span className={styles.topBarFriendName}>{friendDisplayName}</span>
